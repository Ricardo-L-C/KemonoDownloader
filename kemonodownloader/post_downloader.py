...
from PyQt6.QtGui import QColor, QMovie, QPixmap
...
class MediaPreviewModal(QDialog):
    ...
    def init_ui(self):
        ...
        self.display_combo.addItems(
            [translate("fit"), translate("stretch"), translate("original"), translate("full_screen")]
        )
    ...
    def adjust_dialog_size(self):
        if self.display_mode == "Original" and self.original_size:
            ...
            new_height = min(
                content_size.height() + extra_height, QApplication.primaryScreen().availableSize().height()
            )
    ...
    def run(self):
        ...
        try:
            ...
            if (
                ...
                self.log.emit(
                    translate("log_error", "No valid post data returned! Response: " + json.dumps(post_data, indent=2)),
                    "ERROR",
                )
            ...
            self.log.emit(
                translate("log_info", f"Post fetched for {self.url}: {post.get('title', f'File {post_id}')}"), "INFO"
            )
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt == max_retries - 1:
                    self.log.emit(
                        translate("log_error", f"Request failed after {max_retries} attempts: {str(e)}"), "ERROR"
                    )
                    return None
...
class FilePreparationThread(QThread):
    ...
    def detect_files(self, post, allowed_extensions):
        files_to_download = []
        self.log.emit(
            translate("log_debug", f"Detecting files for post with allowed extensions: {allowed_extensions}"), "INFO"
        )
            ...
            for attachment in post["attachments"]:
                if isinstance(attachment, dict) and "path" in attachment:
                    ...
                    if "f=" not in attachment_url and attachment_name:
                        attachment_url += f"?f={attachment_name}"
                    self.log.emit(
                        translate("log_debug", f"Checking attachment: {attachment_name} ({attachment_ext})"), "INFO"
                    )
    ...
    def fetch_post_data(self, post_id, max_retries=3, retry_delay_seconds=5):
        ...
        for attempt in range(1, max_retries + 1):
            try:
                ...
                if response is None:
                    if attempt < max_retries:
                        self.log.emit(
                            translate(
                                "log_warning",
                                f"Failed to fetch {api_url} (attempt {attempt}/{max_retries}). Retrying...",
                            ),
                            "WARNING",
                        )
                        ...
                        continue
                    self.log.emit(
                        translate("log_error", f"Failed to fetch {api_url} after {max_retries} attempts"), "ERROR"
                    )
                    return None
                ...
                if attempt == max_retries:
                    self.log.emit(
                        translate("log_error", f"Error fetching post {post_id} after {max_retries} attempts: {str(e)}"),
                        "ERROR",
                    )
                    return None
                self.log.emit(
                    translate(
                        "log_warning",
                        f"Error fetching post {post_id} (attempt {attempt}/{max_retries}): {str(e)}. Retrying...",
                    ),
                    "WARNING",
                )
                for i in range(retry_delay_seconds, 0, -1):
...
class DownloadThread(QThread):
            ...
            try:
                ...
                with open(full_path, "wb") as f:
                    for chunk in response.iter_content(chunk_size=8192):
                        if not self.is_running:
                            self.log.emit(
                                translate("log_warning", translate("download_interrupted", file_url)), "WARNING"
                            )
                ...
                if attempt == max_retries:
                    self.log.emit(
                        translate(
                            "log_error", translate("error_downloading_after_retries", file_url, max_retries, str(e))
                        ),
                        "ERROR",
                    )
                    ...
                    for i in range(3, 0, -1):
                        if not self.is_running:
                            self.log.emit(
                                translate("log_info", f"Retry for {file_url} cancelled during countdown"), "INFO"
                            )
                            return
    ...
    def run(self):
        ...
        if total_files > 0:
            with ThreadPoolExecutor(max_workers=self.max_concurrent) as executor:
                futures = {
                    executor.submit(self.download_file, file_url, self.download_folder, i, total_files): i
...
class PostDownloaderTab(QWidget):
    ...
    def check_post_from_queue(self, url):
        if not isinstance(url, str):
            self.append_log_to_console(
                translate("log_error", f"Invalid URL type: {type(url)}. Expected string."), "ERROR"
            )
            return
        ...
        if url in self.all_files_map:
            ...
            self.append_log_to_console(
                translate("log_debug", f"Total detected posts: {len(self.all_detected_posts)}"), "INFO"
            )
    ...
    def display_files_for_post(self, url):
        ...
        try:
            ...
            if not response or response.status_code != 200:
                self.append_log_to_console(
                    translate("log_error", f"Failed to fetch {api_url} - No valid response"), "ERROR"
                )
                return
            post_data = self.parse_response_content(response)
            post = post_data if isinstance(post_data, dict) and "post" not in post_data else post_data.get("post", {})
            ...
            self.append_log_to_console(
                translate("log_error", f"Error fetching files for post {url}: {str(e)}"), "ERROR"
            )
    ...
    def on_files_detected_during_check_all(self, detected_files):
        ...
        self.append_log_to_console(
            translate("log_debug", f"Files detected so far: {len(self.files_to_download)}"), "INFO"
        )
    ...
    def on_check_all_posts_detected(self, url, posts):
        ...
        if not any(thread.isRunning() for thread in self.active_threads if isinstance(thread, PostDetectionThread)):
            ...
            self.append_log_to_console(
                translate(
                    "log_info",
                    f"Finished checking all posts. Total posts: {total_posts}, Total files: {len(self.files_to_download)}",
                ),
                "INFO",
            )
            self.detected_files_during_check_all = []
    ...
    def start_post_download(self):
        ...
        if self.download_all_links.isChecked():
            ...
            self.append_log_to_console(
                translate("log_info", translate("preparing_files_post", self.current_post_url)), "INFO"
            )
    ...
    def prepare_files_for_download(self, urls):
        ...
        if not post_ids:
            self.append_log_to_console(
                translate("log_warning", f"No posts available for download in URLs: {urls}. Skipping to next."),
                "WARNING",
            )
        ...
        self.file_preparation_thread.finished.connect(
            lambda files, files_map: self.cleanup_thread(self.file_preparation_thread, [])
        )
    ...
    def on_file_preparation_finished(self, urls, files_to_download, files_to_posts_map):
        self.append_log_to_console(
            translate("log_debug", f"Files prepared for URLs: {urls}, Total files: {len(files_to_download)}"), "INFO"
        )
        for file_url in files_to_download:
            if file_url not in self.checked_urls:
                self.checked_urls[file_url] = True
        self.append_log_to_console(
            translate("log_debug", f"Updated checked_urls after preparation: {self.checked_urls}"), "INFO"
        )
        ...
        self.append_log_to_console(
            translate("log_debug", f"Checked files after filtering: {len(checked_files)}"), "INFO"
        )
        self.append_log_to_console(translate("log_debug", f"Checked files list: {checked_files}"), "INFO")

        if not checked_files:
            self.append_log_to_console(
                translate("log_warning", f"No files to download for URLs: {urls}. Proceeding to next post."), "WARNING"
            )
            self.process_next_post(urls[1:] if len(urls) > 1 else [])
            return
        ...
        self.append_log_to_console(
            translate("log_info", f"Processing post: {url}, Remaining URLs: {remaining_urls}"), "INFO"
        )
        parts = url.split("/")
        post_id = parts[-1]
    ...
    def process_next_post(self, remaining_urls):
        self.append_log_to_console(
            translate("log_info", f"Processing next post. Remaining URLs: {remaining_urls}"), "INFO"
        )
    ...
    def cleanup_thread(self, thread, remaining_urls):
        self.append_log_to_console(
            translate("log_info", f"Cleaning up thread for post. Remaining URLs: {remaining_urls}"), "INFO"
        )
        if thread in self.active_threads:
            ...
            self.append_log_to_console(
                translate(
                    "log_debug",
                    f"Removed thread from active_threads. Active threads remaining: {len(self.active_threads)}",
                ),
                "INFO",
            )
        else:
        ...
        self.append_log_to_console(
            translate("log_debug", f"Active DownloadThreads remaining: {len(active_download_threads)}"), "INFO"
        )

        if not active_download_threads:
            self.append_log_to_console(
                translate("log_info", "No active DownloadThreads remaining. Proceeding to next post."), "INFO"
            )
            ...
            self.append_log_to_console(
                translate(
                    "log_debug",
                    f"Active DownloadThreads still running: {len(active_download_threads)}. Waiting before proceeding.",
                ),
                "INFO",
            )
        ...
        if self.active_threads:
            ...
            for thread in self.active_threads[:]:
                if thread.isRunning():
                    ...
                    self.active_threads.remove(thread)
                    self.append_log_to_console(
                        translate("log_info", f"Terminated thread: {thread.__class__.__name__}"), "INFO"
                    )
            self.post_file_progress.setStyleSheet(
    ...
    def update_file_completion(self, file_index, file_url):
        if file_url not in self.completed_files:
            ...
            self.append_log_to_console(
                translate(
                    "log_debug",
                    f"File completed: {file_url}, Total completed: {len(self.completed_files)}/{self.total_files_to_download}",
                ),
                "INFO",
            )
    ...
    def toggle_check_all(self, state):
        ...
        self.update_checked_files()
        self.append_log_to_console(
            translate("log_debug", f"Check ALL toggled to {is_checked} for {len(visible_urls)} visible files"), "INFO"
        )
    ...
    def update_checked_files(self):
        ...
        if visible_urls:  # If there are visible items, only include checked files from those
            self.files_to_download = [file_url for file_url in visible_urls if self.checked_urls.get(file_url, False)]
    ...
    def filter_items(self):
        ...
        for file_name, file_url in self.all_detected_files:
            ...
            if (not search_text or search_text in file_name.lower()) and (
                not active_filters or file_ext in active_filters or (file_ext == ".jpeg" and ".jpg" in active_filters)
            ):
                self.add_list_item(file_name, file_url)
    ...
    def toggle_checkbox_state(self, url):
        ...
        self.append_log_to_console(
            translate(
                "log_debug", f"Checkbox toggled for {url} to {new_state}, checked_urls count: {len(self.checked_urls)}"
            ),
            "INFO",
        )
        self.update_checked_files()
    ...
    def view_current_item(self):
        if self.current_preview_url:
            ...
            if ext in unsupported_extensions:
                self.append_log_to_console(
                    translate("log_warning", translate("preview_not_supported", ext, self.current_preview_url)),
                    "WARNING",
                )
                return
